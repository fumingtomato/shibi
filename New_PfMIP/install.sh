...
# Version: 16.0.0
# Fixed: Complete module loading with retry logic and better error handling
# =================================================================
...
safe_source() {
    ...
    # Source with timeout protection
    (
        # Set timeout for sourcing
        ( sleep $MODULE_LOAD_TIMEOUT && kill -TERM $$ ) 2>/dev/null &
        local timeout_pid=$!
        
        # Source the module
        source "$module_file"
        
        # Kill timeout if source succeeded
...
check_requirements() {
    ...
    local required_commands=("curl" "wget" "unzip" "systemctl" "git")
    ...
    if [ ${#missing_commands[@]} -gt 0 ]; then
        ...
        apt-get update
        apt-get install -y curl wget unzip systemd git
    fi
}
...
download_all_modules() {
    ...
    # Define all modules with their GitHub names (using hyphens)
    declare -A modules=(
        ...
        ["main_installer_part2"]="main_installer_part2"
    )
...
load_all_modules() {
    ...
    for func in "${critical_functions[@]}"; do
        if ! type "$func" &>/dev/null 2>&1; then
            print_error "Critical function not loaded: $func"
            return 1
        fi
    done
...
cleanup_on_exit() {
    ...
    if [ -f /root/.installer_progress ]; then
        cp /root/.installer_progress /root/.installer_progress.backup
    fi
    
    # Clear temporary files
    rm -f /tmp/installer_*.tmp 2>/dev/null
