...
-- FIX: Increase password column size to 255 to accommodate SHA512-CRYPT hashes
...
setup_dovecot() {
    ...
    mkdir -p /var/vmail
    chmod 770 /var/vmail
    chown vmail:vmail /var/vmail  # FIX: Changed dovecot to vmail group
...
EOF
    ...
    # FIX: Check if SSL certificates exist, if not, use a temporary self-signed certificate for initial setup
    if [ ! -d "/etc/letsencrypt/live/$hostname" ] || [ ! -f "/etc/letsencrypt/live/$hostname/fullchain.pem" ]; then
        print_message "SSL certificates not found. Creating temporary self-signed certificate for initial setup..."
        
        # Create directory for temporary certificates
        mkdir -p /etc/dovecot/ssl
        
        # Generate self-signed certificate
        openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
            -keyout /etc/dovecot/ssl/dovecot.key -out /etc/dovecot/ssl/dovecot.crt \
            -subj "/CN=$hostname/O=Mail Server/C=US"
        
        # Configure SSL with temporary certificates
        cat > /etc/dovecot/conf.d/10-ssl.conf <<EOF
# SSL configuration with temporary certificates
# Generated by Mail Server Installer
...
EOF
        ...
        # Configure SSL with Let's Encrypt certificates
        cat > /etc/dovecot/conf.d/10-ssl.conf <<EOF
# SSL configuration
# Generated by Mail Server Installer
