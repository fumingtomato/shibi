...
run_system_updates() {
    ...
    if [ "$ENABLE_AUTOMATIC_UPDATES" != "false" ]; then
        update_system
    else
        print_message "Automatic updates are disabled in settings. Skipping."
...
update_system() {
    ...
    if [ -f /var/lib/apt/periodic/update-success-stamp ]; then
        ...
        if [ $hours_since_update -lt 24 ]; then
            ...
            log "System was updated less than 24 hours ago. Skipping update."
            return
        fi
    fi
...
EOF
        ...
        log "Unattended upgrades configuration completed"
    else
        print_message "Unattended upgrades are already configured."
        log "Unattended upgrades are already configured"
    fi
}
